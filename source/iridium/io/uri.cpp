    ...
    m_source(source)
{
    ...
    if (m_source.empty())
        throw std::runtime_error("wrong uri '" + m_source + "'"); // ----->
    ...
    if ((m_host.empty() || !m_ipv4) && m_port)
        throw std::runtime_error("wrong uri '" + m_address + "'"); // ----->
}


URI::TIPv4 URI::getIPv4() const {
    if(!m_ipv4)
...
iridium::io::URI::TIPv4 convertStringToIPv4(string const &ipv4) {
    ...
    try {
        ...
        size_t i = 0;
        for (auto byte: assertSize(iridium::split(ipv4, "."), 4, "expected 4 numbers"))
