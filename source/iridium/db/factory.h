...
///     Parses the given URI and extracts database connection parameters (like type, host, port, user, password, database name)
///     to populate a `config::TDatebase` object.
/// \~russian @brief Создает объект конфигурации базы данных из URI.
///     Разбирает данный URI и извлекает параметры подключения к базе данных (такие как тип, хост, порт, пользователь, пароль, имя базы данных)
///      для заполнения объекта `config::TDatebase`.
...
///     Схема URI может определять тип базы данных (например, "mysql://", "postgres://").
...
///     This function acts as a factory to instantiate a concrete `IConnector` implementation
...
///     Эта функция действует как фабрика для создания конкретной реализации `IConnector`
///     (например, коннектора MySQL, PostgreSQL), подходящей для данной конфигурации.
...
///     Поле `Type` в конфигурации определяет, какой тип коннектора будет создан.
...
///     Возвращает нулевой указатель или выбрасывает исключение, если коннектор не может быть создан для данной конфигурации.
...
///     This is a convenience function that first calls `makeConfig(uri)` to create a configuration
...
///     Это вспомогательная функция, которая сначала вызывает `makeConfig(uri)` для создания объекта конфигурации
///     из URI, а затем вызывает `createConnector(config)` с этой конфигурацией.
/// \~english @param uri The URI containing database connection information.
/// \~russian @param uri URI, содержащий информацию о подключении к базе данных.
/// \~english @return A shared pointer (`IConnector::TSharedPtr`) to an `IConnector` instance.
///     Returns a null pointer or throws an exception if a connector cannot be created.
/// \~russian @return Умный указатель (`IConnector::TSharedPtr`) на экземпляр `IConnector`.
///     Возвращает нулевой указатель или выбрасывает исключение, если коннектор не может быть создан.
