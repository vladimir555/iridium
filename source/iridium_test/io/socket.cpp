...
TEST(uri) {
    ...
    ASSERT(URI("172.16.0.64") , std::exception);
...
//    ASSERT({ 172, 16, 0, 64 }, equal, uri.getIPv4());
    ...
    // test dns resolver
    ASSERT(!URI("http://ya.ru").getIPv4().empty());
    ASSERT( static_cast<size_t>(7), less, URI("http://ya.ru").getIPv4().size());
