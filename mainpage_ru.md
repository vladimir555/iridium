...
2.  **Проверка на сравнение: `ASSERT(value1, comparison_symbol, value2)`**
    ...
    -   Важно отметить, что в процессе сравнения `value2` может быть приведен к типу `value1`. Это преобразование типов осуществляется с помощью функции `iridium::convertion::convert`.
